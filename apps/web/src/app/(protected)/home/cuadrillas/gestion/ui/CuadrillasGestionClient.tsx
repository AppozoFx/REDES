...
export default function CuadrillasGestionClient() {
  ...
  const saveEdit = async (row: CuadrillaRow) => {
    ...
    const payload = {
      ...
      credUsuario: form.credUsuario ?? "",
      credPassword: form.credPassword ?? "",
        ...
        <table className="min-w-full text-sm">
          <thead className="bg-slate-100 text-gray-700 dark:bg-slate-800 dark:text-slate-200">
            <tr>
              ...
              <th className="p-2 text-left">Credencial usuario</th>
              <th className="p-2 text-left">Credencial password</th>
          ...
          <tbody>
                ...
                <tr key={row.id} className="border-t border-slate-200 hover:bg-slate-50 dark:border-slate-700 dark:hover:bg-slate-800/70">
                  ...
                  <td className="p-2 min-w-[180px]">
                    {editing ? (
                      <input
                        className="w-full rounded border border-slate-300 px-2 py-1 dark:border-slate-700 dark:bg-slate-950 dark:text-slate-100"
                        value={form.credUsuario || ""}
                        onChange={(e) => setForm((p) => ({ ...p, credUsuario: e.target.value }))}
                        placeholder="Usuario credencial"
                      />
                    ) : (
                      row.credUsuario || "-"
                    )}
                  ...
                  <td className="p-2 min-w-[180px]">
                    {editing ? (
                      <input
                        ...
                        onChange={(e) => setForm((p) => ({ ...p, credPassword: e.target.value }))}
                        placeholder="Password credencial"
                      />
                    ) : (
                      row.credPassword || "-"
                    )}
            ...
            {filtered.length === 0 && (
              <tr>
                <td colSpan={11} className="p-6 text-center text-gray-500 dark:text-slate-400">
