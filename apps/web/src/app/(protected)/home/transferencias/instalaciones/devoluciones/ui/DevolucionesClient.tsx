...
/**
 * Mantiene tu lgica:
 * - Server Action: devolverInstalacionesAction
...
export default function DevolucionesClient() {
  ...
  useEffect(() => {
    ...
    if ((result as any).ok) {
      const r: any = result;
      toast.success("Devolucion generada", { description: `Gua: ${r.guia}` });
      ...
      if (r.guia && printedGuiaRef.current !== r.guia) {
        ...
        (async () => {
          ...
          if (ok) resetDevolucionForm();
        })();
      }
    } else {
      const msg = (result as any)?.error?.formErrors?.join(", ") || "Error en devolucion";
      toast.error(msg);
    }
  }, [result]);
  ...
  async function imprimirGuiaTermica(): Promise<boolean> {
    ...
    const path = `guias/instalaciones/devolucion/${guia}.pdf`;
    ...
    try {
      ...
      const res = await fetch(
        `/api/transferencias/instalaciones/guia/upload?guiaId=${encodeURIComponent(guia)}&tipo=devolucion&token=${encodeURIComponent(token)}`,
        {
          method: "POST",
