...
function buildActionHint(reason: LookupReason, targetCuadrillaNombre: string): string {
  ...
  if (reason === "ALREADY_INSTALLED") return "SN ya figura como instalado.";
  if (reason === "NOT_FOUND") return "Verificar el serial ingresado.";
  if (reason === "IN_ALMACEN") return "El equipo sigue en almacén.";
  if (reason === "IN_GARANTIA" || reason === "IN_AVERIA") return "Equipo no disponible para liquidación.";
  if (reason === "IN_WIN") return "Revisar con WIN antes de liquidar.";
  if (reason === "IN_PERDIDO" || reason === "IN_ROBO") return "No corresponde liquidar este SN.";
  return "Revisar la ubicación real del equipo antes de liquidar.";
}
  ...
  try {
    ...
    if (!equipoSnap.exists) {
      return NextResponse.json({
        ok: true, sn, found: false, equipo: "", inTargetCuadrillaStock: false,
        isInstalado: false, reason: "NOT_FOUND", actionHint: "Verificar el serial ingresado.",
      });
    }
