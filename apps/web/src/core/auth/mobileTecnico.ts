...
export async function getTecnicoContext(mobile: MobileAuthContext) {
  ...
  const integrantes = tecnicoSnaps.map((snap: any) => {
    const data = snap.exists ? (snap.data() as any) : {};
    const nombre = `${String(data?.nombres || "").trim()} ${String(data?.apellidos || "").trim()}`.trim() || snap.id;
    return {
      uid: snap.id,
      nombre,
    };
...
export async function listTecnicoOrders(cuadrillaId: string, ymd: string) {
  ...
  return Array.from(docsById.entries()).map(([id, data]) => {
    ...
    return {
      ...
      codigoCliente: String(data?.codiSeguiClien || "").trim(),
...
export async function getTecnicoHomeData(cuadrillaId: string) {
  ...
  const instalacionesMes = monthOrders.filter((item) => !item.isGarantia);
  const garantiasMes = monthOrders.filter((item) => item.isGarantia);
  const finalizadasMes = monthOrders.filter((item) => item.isFinalizada).length;
  const liquidadasMes = monthOrders.filter((item) => item.isLiquidated).length;
  const pendientesMes = monthOrders.length - finalizadasMes;
  ...
  return {
    fecha: today,
    kpis: {
      totalOrdenesMes: monthOrders.length,
      instalacionesMes: instalacionesMes.length,
      garantiasMes: garantiasMes.length,
      finalizadasMes,
      liquidadasMes,
      pendientesMes,
    },
...
export async function getTecnicoStock(cuadrillaId: string) {
  ...
  const [equiposSnap, materialesSnap] = await Promise.all([
  ...
  const equipos = equiposSnap.docs.map((doc) => {
    ...
    return {
      ...
      tipo: String(data?.equipo || "").trim(),
      proid: String(data?.proId || data?.proid || "").trim(),
    };
  }).sort((a, b) => a.tipo.localeCompare(b.tipo) || a.sn.localeCompare(b.sn));
