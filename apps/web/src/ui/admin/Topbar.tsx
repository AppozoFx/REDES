...
export default function Topbar({ uid }: Props) {
  ...
  return (
    <div className="flex items-center justify-between rounded-2xl border border-slate-200/80 bg-white px-3 py-2 shadow-sm">
      <div className="flex items-center gap-3">
        <Link href="/admin" className="inline-flex h-8 w-8 items-center justify-center overflow-hidden rounded-xl bg-white ring-1 ring-slate-200">
          <Image src="/img/logo.png" alt="Logo REDES" width={24} height={24} className="h-6 w-6 object-contain" />
        </Link>
        <span className="text-sm font-medium text-slate-600">Panel Admin</span>
      </div>
      ...
      <div className="flex items-center gap-3">
        ...
        <div ref={menuRef} className="relative">
          ...
          >
            <div className="mb-1 flex items-center gap-2 rounded-lg bg-slate-50 px-2 py-2">
              <div className="inline-flex h-7 w-7 items-center justify-center rounded-full bg-[#30518c] text-[11px] font-semibold text-white">
                {initials}
              </div>
              <div className="min-w-0 text-xs font-medium text-slate-700">{identidad}</div>
            </div>
