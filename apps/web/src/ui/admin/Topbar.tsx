...
export default function Topbar({ uid }: Props) {
  ...
  return (
    <div className="flex items-center justify-between rounded-2xl border border-slate-200/80 bg-white px-3 py-2 shadow-sm dark:border-slate-700 dark:bg-slate-900">
      <div className="flex items-center gap-3">
        <Link href="/admin" className="inline-flex h-8 w-8 items-center justify-center overflow-hidden rounded-xl bg-white ring-1 ring-slate-200 dark:bg-slate-800 dark:ring-slate-700">
          <Image src="/img/logo.png" alt="Logo REDES" width={24} height={24} className="h-6 w-6 object-contain" />
        </Link>
        <span className="text-sm font-medium text-slate-600 dark:text-slate-300">Panel Admin</span>
      </div>
      ...
      <div className="flex items-center gap-3">
        ...
        <div ref={menuRef} className="relative z-[160]">
          ...
          >
            ...
            <button
              className="block w-full rounded-lg px-3 py-2 text-left text-sm text-rose-700 hover:bg-rose-50 dark:text-rose-300 dark:hover:bg-rose-950/40"
              onClick={async () => {
                setMenuOpen(false);
