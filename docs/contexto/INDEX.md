...
| Cierre de cuadrillas Winbo | En progreso, bloqueado | `apps\web\src\lib\winbo\cuadrillasCierre.ts`, `apps\web\src\app\api\cuadrillas\winbo\cerrar`, `apps\web\src\app\(protected)\home\cuadrillas\cierre-winbo` | `web/winbo-cierre-cuadrilla.md` | Alta | Backend + UI escritos y probados en vivo; bloqueado por `CUADRILLA_NO_ENCONTRADA_WINBO` en cuadrillas activas, pendiente confirmar alcance de cuenta `WINBO_USERNAME` |
