...
| Cierre de cuadrillas Winbo | En progreso, bloqueado | `apps\web\src\lib\winbo\cuadrillasCierre.ts`, `apps\web\src\app\api\cuadrillas\winbo\cerrar`, `apps\web\src\app\(protected)\home\cuadrillas\cierre-winbo` | `web/winbo-cierre-cuadrilla.md` | Alta | `EsHorarioValido` ya no bloquea (bajado a advertencia); sigue bloqueado por `CUADRILLA_NO_ENCONTRADA_WINBO`, ahora con hipotesis de colision de sesiones concurrentes (misma cuenta WinBo en local/produccion) |
